#Entropy correction, kalman Filter and interpolation, Log transformation, and first Differencing (ENFoLD)
...
x_ec = x.multiply(1.0/x_esc.iloc[:,0],axis=0)
...
for i in range(x_ec_int_kf_log.shape[1]):
    ...
    for x in range(x_ec_int_kf_log.shape[0] - 1):
        log_delta = x_ec_int_kf_log.iloc[x+1,i] - x_ec_int_kf_log.iloc[x,i]
        log_delta_list.append(log_delta)
    x_ec_int_kf_log_delta[x_ec_int_kf_log.columns[i]] = np.transpose(log_delta_list)

print "First differences successfully calculated."


nan_file_handle = '%s_ec_nan.txt' % args.o
kf_file_handle = '%s_ec_kf.txt' % args.o
kf_log_file_handle = '%s_ec_kf_log.txt' % args.o
kf_log_delta_file_handle = '%s_ec_kf_log_delta.txt' % args.o 


x_ec_nans.to_csv(nan_file_handle, sep='\t')
x_ec_int_kf.to_csv(kf_file_handle, sep='\t')
x_ec_int_kf_log.to_csv(kf_log_file_handle, sep='\t')
x_ec_int_kf_log_delta.to_csv(kf_log_delta_file_handle, sep='\t')
